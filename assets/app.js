...
BX24.init(function () {
  ...
  BX24.callBatch(
    ...
    function (results) {
      ...
      // переопределяет display обратно на none. Без явного style.display='flex'
      // форма остаётся невидимой, хотя класс уже снят. Inline-стиль имеет
      // максимальный приоритет и гарантирует именно flex-вёрстку, которую
      // ожидают дочерние блоки формы.
      if (form) {
        form.classList.remove('hidden');   // Убираем скрывающий CSS-класс
        form.style.display = 'flex';       // см. комментарий выше — нужен поверх каскада
      }
