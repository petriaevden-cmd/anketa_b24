...
const RULES = [
  // 1. Долг менее 300 000 рублей.
  {
    ...
    predicate: function (f) {
      const debt = _toNumber(f.debtTotal);
  ...
  // 9. Высокий официальный доход (невыгодно по расчёту КМ).
  //    Поле officialIncome хранит число в рублях — сравнение с enum 'high'
  //    больше не работает. Признак нецелевого определяется только чек-боксом
  //    «Невыгодно по расчёту КМ» (f.incomeKmBad === 'Y').
  {
    key: 'high_official_income',
    label: 'Высокий официальный доход',
    predicate: function (f) {
      return f.incomeKmBad === 'Y';
    }
  },
 ...
 *     ooo:              'Y' | 'N' | '',
 *     criminal:         'Y' | 'N' | '',
 *     incomeOfficial:   число в рублях (алиас, в правилах не используется — признак по доходу только через incomeKmBad),
 *
...
function evaluateTargetStatus(formData) {
  ...
  for (let i = 0; i < RULES.length; i++) {
    ...
    try {
      ...
      // Без try/catch одно сломанное правило (например, неожиданный тип
      // в formData) остановило бы цикл по RULES, и весь набор последующих
      // правил вообще не проверился бы — функция вернула бы «Не определено»
      // даже при наличии явных и независимых признаков нецелевого. Это
      // хуже, чем пропустить одно правило: менеджер увидел бы на экране
      // нейтральный статус, который не отражает реальные данные анкеты.
  ...
  const hasDebt = _toNumber(f.debtTotal) !== null && _toNumber(f.debtTotal) > 0;
  // incomeOfficial намеренно исключён: это числовое поле в рублях,
  // любое ненулевое значение было бы truthy и давало ложный «Целевой».
  // Признак нецелевого по доходу проверяется чек-боксом incomeKmBad (правило 9).
  const hasAnyAnswer =
